...
sed $SED_INPLACE 's/""/"/g' "$TMPPATH"

# Remove packages that we build locally
sed $SED_INPLACE '/Xamarin.Tests.FrameworksInRuntimesNativeDirectory/d' "$TMPPATH"
sed $SED_INPLACE '/Xamarin.Tests.DynamicLibrariesInRuntimesNativeDirectory/d' "$TMPPATH"
sed $SED_INPLACE '/Xamarin.Tests.XCFrameworkWithStaticLibraryInRuntimesNativeDirectory/d' "$TMPPATH"
sed $SED_INPLACE '/Xamarin.Tests.XCFrameworkWithSymlinks/d' "$TMPPATH"

# Get only the name and version of each package, and write that back in a PackageDownload item
sed $SED_INPLACE 's@.*<PackageReference.*Include="\([a-zA-Z0-9._-]*\)".*Version="\([a-zA-Z0-9._-]*\)".*>.*@\t\t<PackageDownload Include="\1" Version="[\2]" />@g' "$TMPPATH"
