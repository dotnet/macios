...
1. Retrieve the pull request author. If the author's login is `csigs`, stop immediately without reading the diff or posting any review comments, summary, or verdict. These pull requests are generated by the localization pipeline and must not be reviewed or rubber-stamped.
2. Read the review rules from `.github/skills/code-review/references/review-rules.md` — these contain the detailed patterns and anti-patterns to check for.
3. Read the review methodology from `.github/skills/code-review/SKILL.md` — this defines the review workflow, mindset, severity levels, and comment format.
...
- Do not submit any review event for pull requests authored by `csigs`.
